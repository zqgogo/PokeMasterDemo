//
//  TestMetalTexture2DViewController.swift
//  PokeMasterDemo
//
...
class TestMetalTexture2DViewController: UIViewController {
    ...
    func setupPipeline() {
        ...
        guard let vertexFunction = defaultLibrary.makeFunction(name: "vertexShader_7") else { return }
        guard let fragmentFunction = defaultLibrary.makeFunction(name: "samplingShader_7") else { return }
    ...
    func setupTexture() {
        ...
        texture?.replace(region: region, mipmapLevel: 0, withBytes: imageBytes, bytesPerRow: Int(image.size.width) * 4)
    }
    ...
    private func loadImage(image: UIImage) -> UnsafeMutableRawPointer? {
        ...
        guard let context = CGContext(data: data, width: width, height: height, bitsPerComponent: 8, bytesPerRow: bytesPerRow, space: CGColorSpaceCreateDeviceRGB(), bitmapInfo: CGImageAlphaInfo.premultipliedLast.rawValue) else { return nil }
...
extension TestMetalTexture2DViewController: MTKViewDelegate {
